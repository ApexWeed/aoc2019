...
namespace Day3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var visits = input.Select(FindVisited);
            var collisions = FindCollisions(visits);
            var distances = CalculateDistance(collisions).ToArray();

            Console.WriteLine(distances.OrderBy(d => d.manhattan).First(d => d.manhattan != 0).manhattan);
            Console.WriteLine(distances.OrderBy(d => d.steps).First(d => d.manhattan != 0).steps);
        }

        private static List<(int x, int y, int step)> FindVisited(string input)
        {
            var visited = new List<(int x, int y, int step)>();
            var x = 0;
            var y = 0;
            var step = 0;
            ...
            foreach (var move in input.Split(','))
            {
                ...
                {
                    case 'U':
                        visited.AddRange(Enumerable.Range(1, int.Parse(move.Substring(1))).Select(i => (x, y + i, step + i)));
                        y += int.Parse(move.Substring(1));
                        break;
                    case 'D':
                        visited.AddRange(Enumerable.Range(1, int.Parse(move.Substring(1))).Select(i => (x, y - i, step + i)));
                        y -= int.Parse(move.Substring(1));
                        break;
                    case 'R':
                        visited.AddRange(Enumerable.Range(1, int.Parse(move.Substring(1))).Select(i => (x + i, y, step + i)));
                        x += int.Parse(move.Substring(1));
                        break;
                    case 'L':
                        visited.AddRange(Enumerable.Range(1, int.Parse(move.Substring(1))).Select(i => (x - i, y, step + i)));
                        x -= int.Parse(move.Substring(1));
                ...
                step += Math.Abs(int.Parse(move.Substring(1)));
            }

            return visited;
        }

        private static IEnumerable<(int x, int y, int steps)> FindCollisions(IEnumerable<List<(int x, int y, int step)>> visits)
            => visits.SelectMany(a => a.GroupBy(g => (g.x, g.y))
                                       .Select(g => g.OrderBy(p => p.step)
                                                     .First()))
                     .GroupBy(v => (v.x, v.y))
                     .Where(g => g.Count() > 1)
                     .Select(g => (g.Key.x, g.Key.y, g.Sum(k => k.step)));

        private static IEnumerable<(int manhattan, int steps)> CalculateDistance(IEnumerable<(int x, int y, int steps)> collisions)
            => collisions.Select(c => (Math.Abs(c.x) + Math.Abs(c.y), c.steps));
    }
}
